...
const backendKeypair = Keypair.fromSecret(
  "SCUK4TJBBSKZVOS3ELZGRNGDYEGISJKMIN2KOY6E5XS6JXBQNUKYNGRO",
);
const userKeyPair = Keypair.fromSecret(
  "SDJH3Q25A66PT27RSI2S2TPB6LJ72BZXOFUZU6TK72OW5A77EMLR57AA",
);
const recipientKeypair = Keypair.fromSecret(
  "SCQYME5TXWWFQDBFJ2N3RPCHEUANSAUHCHJHOAELPCGHCD6TXV33NUNY",
);
...
export async function GasolinaMain() {
  ...
  const amountInMaxUsdc = Math.ceil(
    (amountOutXlm * RATE_XLM_TO_USDC + XLM_EXTRA_MARGIN) * SCALE,
  );
  ...
  const unsignedCaller = sim.result?.auth.find(
    (cred) =>
      Address.fromScAddress(
        cred.credentials().address().address(),
      ).toString() === callerG,
  );

  // Both may be required depending on your contract flow.
  if (!unsignedCaller)
    throw new Error("Missing caller auth requirement in simulation result");
